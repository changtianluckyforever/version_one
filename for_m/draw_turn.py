...
def draw(color, marker, linestyle, record_list=range(1,4), model_path="", attribute="ave_turns"):
    datapoints = []
    for i in record_list:
        datapoints.append(
    ...
    l, = plt.plot(range(mean.shape[0]), mean, color, marker=marker, markevery=10, markersize=11, linewidth=linewidth)
...
def main(params):
    ...
    # example
    model_path_list = [
        './deep_dialog/for_plot/pav_0_1',
        './deep_dialog/for_plot/pav_0.1_1',
        './deep_dialog/for_plot/pav_0.2_1'
    ]
    label_list = ['error0','error0.1', 'error0.2']
    ...
    plt.grid(True)
    plt.ylabel('average turns',fontsize = 'xx-small')
