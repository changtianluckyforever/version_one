...
class DialogManager:
    ...
    def initialize_episode(self, evaluation=False, epis=10000):
        ...
        self.user_action = self.user.initialize_episode(evaluation=evaluation, epis=epis)     #  we are here, we need to add evaluation and epis
    ...
    def next_turn(self, record_training_data=True, eval = False):
        ...
        self.agent_action = self.agent.state_to_action(self.state, eva= eval)
        if eval:
    ...
    def reward_function(self, dialog_status):
        ...
        if dialog_status == dialog_config.FAILED_DIALOG:
            reward = -self.user.max_turn    #-self.user.max_turn #10
