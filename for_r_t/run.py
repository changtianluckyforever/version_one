...
from deep_dialog.dialog_system import DialogManager, text_to_dict     # module name
...
if __name__ == "__main__":
    ...
    parser.add_argument('--evaluation', dest='evaluation', type=bool, default=False, help='whether we use the fixed test set')
    ...
    parser.add_argument('--save_check_point', dest='save_check_point', type=int, default=30, help='number of epochs for saving model')
    ...
    parser.add_argument('--dpav', type=int, default=0)
    parser.add_argument('--averaged', type=int, default=0)
    ...
    parser.add_argument('--discount_factor', type=float, default=0.10, help = "the lambda value for Q min")
    parser.add_argument('--num_target_net', type=int, default=4)
...
discount_val = params['discount_factor']
agt = params['agt']
usr = params['usr']
...
agent_params['averaged'] = params['averaged']
...
agent_params['dpav'] = params['dpav']
agent_params['maxmin_dqn'] = params['maxmin']
agent_params['bayesian_dqn'] = params['bayesian']
...
if agt == 0:
    ...
    agent = AgentDQN(kb, act_set, slot_set, agent_params, discount_val=discount_val, count_num=0)
...
def simulation_epoch(simulation_epoch_size, evaluation = False, train=False):
    ...
    for episode in xrange(simulation_epoch_size):
        dialog_manager.initialize_episode(evaluation=evaluation, epis=episode)
        episode_over = False
        mark_time = 1
        while(not episode_over):
            ...
            if train and step % 1 == 0:
                discount_val, count_num = agent.record_progress()
                if agent.dpav or agent.averaged or agent.maxmin_dqn:
                    err, i_r = agent.train(batch_size, 1, discount_val=discount_val, count_num=count_num)
                    if agent.dpav:
                        print("******")
                        print('discount value is:', discount_val)
                        print('count num is:', count_num)
                        print('******')
                else:
                    err, i_r = agent.train(batch_size, 1)
                # err, i_r = agent.train(batch_size, 1, discount_val= discount_val)
                loss += err
                intrinsic_reward += i_r
                update_count += 1   # after every training step, it will accumulate one
    if train:
...
def run_episodes(count, status, evaluation = False):
    print('current evaluation value is:', evaluation)
    successes = 0
    cumulative_reward = 0
    cumulative_turns = 0
    ...
    for episode in xrange(count):
        ...
        if (agt == 9 or agt == 12 or agt == 13) and params['trained_model_path'] == None:
            agent.predict_mode = True       # when the value is true, we begin to increase experience pool
            simulation_res, aaa = simulation_epoch(simulation_epoch_size=50, evaluation=False, train=True)  # we should increase the number from 10 to other numbers.
            performance_records['ave_intrinsic_reward'][episode] = simulation_res['ave_intrinsic_reward']
            agent.predict_mode = False
            
            simulation_res, q_samples = simulation_epoch(simulation_epoch_size, evaluation=evaluation, train=False)
            ...
            if simulation_res['success_rate'] >= best_res['success_rate']:
                if simulation_res['success_rate'] >= success_rate_threshold: # threshold = 0.30
                    ...
                    simulation_epoch(simulation_epoch_size, evaluation = False, train=False)
                    agent.predict_mode = False
    ...
    if (agt == 9 or agt == 12 or agt == 13)  and params['trained_model_path'] == None:
        ...
        save_performance_records(params['write_model_dir'], agt, performance_records)
        save_json_file(params['write_model_dir'], agt, q_result)   #working file now.
    
run_episodes(num_episodes, status, evaluation)
