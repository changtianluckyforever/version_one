...
class AgentDQN(Agent):
    def __init__(self, movie_dict=None, act_set=None, slot_set=None, params=None, discount_val=0.4, count_num=0):
        ...
        self.per = params['per']
        self.dpav = params['dpav']
        self.averaged = params['averaged']
        self.double_dqn = params['double_dqn']
        self.dueling_dqn = params['dueling_dqn']
        self.maxmin_dqn = params['maxmin_dqn']
        self.bayesian_dqn = params['bayesian_dqn']
        self.sunrise = params['sunrise']
        self.batch_size = params['batch_size']
        self.num_target_net = params['num_target_net']
        self.net_parameter = params['net_parameter']
        ...
        if params['distributional']:
            ...
            self.dqn = DQN(self.state_dimension, self.hidden_size, self.num_actions, params['dueling_dqn'],
                params['double_dqn'], params['icm'], params['noisy'], params['dpav'], params['averaged'], params['maxmin_dqn'], params['bayesian_dqn'], params['num_target_net'], params['net_parameter'], params['sunrise'], params['batch_size'])
    ...
    def calculate_q_max(self, state, eva=False):
        self.representation = self.prepare_state_representation(state)

        if eva:
            act, q_max = self.dqn.predict(self.representation, {}, predict_model=True, get_q= True)
            return q_max
        else:
            return None
        
    def state_to_action(self, state, eva = False):
        """ DQN: Input state, output action """
        
        self.representation = self.prepare_state_representation(state)
        self.action = self.run_policy(self.representation, eval= eva)
    ...
    def run_policy(self, representation, eval = False):
        ...
        if random.random() < self.epsilon:
            ...
            if self.warm_start == 1:
                ...
                return self.dqn.predict(representation, {}, predict_model=True, eval= eval)
    ...
    def train(self, batch_size=1, num_batches=100, discount_val =0.4, count_num = 0):
        ...
        for iter_batch in range(num_batches):
            ...
            batch_struct = self.dqn.singleBatch(batch, {'gamma': self.gamma}, discount_val, count_num)
    ...
    def record_progress(self):
        ...
        # print('we are here 2')
        if self.count_num % 30 ==0:
            if self.count_num < 6000:
                self.discount_val = self.discount_val * 0.996     #* 0.996
            elif self.count_num < 18000:
                self.discount_val = self.discount_val * 0.997           # 0.998
            else:
                self.discount_val = self.discount_val * 0.999   #995
